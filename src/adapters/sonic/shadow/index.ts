...
import type { TContractToken, TContractTokenMetadata, TMarketToken, TToken } from '../../../types/tokens.d'
import type {
  TShadowEpochData,
...
function wrapTokenAddressType(token: TToken): TContractTokenMetadata {
  if (token.type === 'native')
    return findTokenByAddress(WRAPPED_S)
  // TODO: Dynamic fetching
  if (token.symbol === 'stS')
    return findTokenByAddress(WRAPPED_S)
  if (token.symbol.includes('ETH'))
    return findTokenByAddress(WETH_S)
  return token
}
...
export class ShadowExchangeApiAdapter implements IMarketDataAdapter {
  ...
  async fetchTokenWithPrice(
    ...
    if (!tokenPriceData || Number(tokenPriceData.priceUSD) === 0)
